...
class DeckExportServiceTests {

    private static final Logger logger = LoggerFactory.getLogger(DeckExportServiceTests.class);
    ...
    @Test
    @DisplayName("Should export deck with empty description")
    void testExportDeckWithEmptyDescription() {
        logger.debug("Test: Export deck with empty description");

        when(testDeck.getDescription()).thenReturn("");
        when(deckService.getDeckOrThrow(DECK_ID, USER_ID)).thenReturn(testDeck);
        when(cardService.getDeckCards(USER_ID, DECK_ID)).thenReturn(Collections.emptyList());
        when(tagService.getDeckTags(USER_ID, DECK_ID)).thenReturn(Collections.emptyList());
        ...
        logger.debug("Test passed: Deck with empty description exported");
    }
