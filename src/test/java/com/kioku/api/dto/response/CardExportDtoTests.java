...
class CardExportDtoTests {

    private static final Logger logger = LoggerFactory.getLogger(CardExportDtoTests.class);
    ...
    void testEntityConstructorWithoutTags() {
        ...
        when(mockCard.getCardId()).thenReturn(CARD_ID);
        when(mockCard.getFront()).thenReturn(FRONT);
        when(mockCard.getBack()).thenReturn(BACK);
        when(mockCard.getNotes()).thenReturn(NOTES);
        when(mockCard.getTags()).thenReturn(Collections.emptySet());
        when(mockCard.getCreatedAt()).thenReturn(testTime);
        when(mockCard.getUpdatedAt()).thenReturn(testTime);

        CardExportDto dto = new CardExportDto(mockCard);
    ...
    void testTagOrderPreserved() {
        ...
        Tag mockTag1 = mock(Tag.class);
        when(mockTag1.getName()).thenReturn("alpha");

        Tag mockTag2 = mock(Tag.class);
        when(mockTag2.getName()).thenReturn("beta");

        Tag mockTag3 = mock(Tag.class);
        when(mockTag3.getName()).thenReturn("gamma");
