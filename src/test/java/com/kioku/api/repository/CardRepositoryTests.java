...
class CardRepositoryTests {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CardRepositoryTests.class);
    ...
    void setUp() {
        ...
        testDeck = new Deck(DECK_NAME, DECK_DESCRIPTION);
    ...
    void testSaveCard() {
        ...
        Card card = new Card(CARD_FRONT_1, CARD_BACK_1);
        ...
        logger.debug("Test passed: Card saved with id={}", savedCard.getCardId());
    }
    ...
    void testFindById() {
        ...
        Optional<Card> found = cardRepository.findById(savedCard.getCardId());

        assertTrue(found.isPresent());
        assertEquals(CARD_FRONT_1, found.get().getFront());

        logger.debug("Test passed: Card found by id={}", savedCard.getCardId());
    }

    /**
     * Tests deleting a card by removing it from the deck.
    ...
    void testDeleteCard() {
        ...
        // Reload deck to get fresh collection with proper ID-based hash codes
        testDeck = deckRepository.findById(testDeck.getId()).orElseThrow();

        // Get the saved card from the deck's cards collection
        Card savedCard = testDeck.getCards().iterator().next();
        Long cardId = savedCard.getCardId();
        assertNotNull(cardId, "Card ID should be assigned after save");

        // Remove from deck - orphanRemoval will delete the card
        testDeck.removeCard(savedCard);
        deckRepository.save(testDeck);
        entityManager.flush();
        entityManager.clear();
    ...
    void testFindByDeckId() {
        ...
        Card card1 = new Card(CARD_FRONT_1, CARD_BACK_1);
        Card card2 = new Card(CARD_FRONT_2, CARD_BACK_2);
    ...
    void testFindByIdAndDeckId() {
        ...
        Optional<Card> found = cardRepository.findByIdAndDeckId(savedCard.getCardId(), testDeck.getId());
    ...
    void testFindByIdAndWrongDeckId() {
        ...
        Optional<Card> found = cardRepository.findByIdAndDeckId(savedCard.getCardId(), 999L);
    ...
    void testSameCardDifferentDecks() {
        ...
        Deck otherDeck = new Deck("Other Deck", "Description");
        testUser.addDeck(otherDeck);
        otherDeck = deckRepository.save(otherDeck);

        Card card1 = new Card(CARD_FRONT_1, CARD_BACK_1);
        Card card2 = new Card(CARD_FRONT_1, CARD_BACK_1);
        testDeck.addCard(card1);
    ...
    void testSearchByFront() {
        ...
        Card card1 = new Card("食べる", "to eat");
        Card card2 = new Card("飲む", "to drink");
        Card card3 = new Card("食事", "meal");
        testDeck.addCard(card1);
        testDeck.addCard(card2);
    ...
    void testSearchByBack() {
        ...
        Card card1 = new Card("食べる", "to eat");
        Card card2 = new Card("飲む", "to drink");
        Card card3 = new Card("読む", "to read");
        testDeck.addCard(card1);
        testDeck.addCard(card2);
        testDeck.addCard(card3);
        cardRepository.save(card1);
    ...
    void testSearchCaseInsensitive() {
        ...
        Card card = new Card(CARD_FRONT_1, "To Eat");
        testDeck.addCard(card);
        cardRepository.save(card);
    ...
    void testSearchPartialMatch() {
        ...
        Card card = new Card(CARD_FRONT_1, "to eat something");
        testDeck.addCard(card);
        cardRepository.save(card);
    ...
    void testFindByDeckIdAndTagId() {
        ...
        Card card1 = new Card(CARD_FRONT_1, CARD_BACK_1);
        Card card2 = new Card(CARD_FRONT_2, CARD_BACK_2);
        Card card3 = new Card(CARD_FRONT_3, CARD_BACK_3);
        testDeck.addCard(card1);
        testDeck.addCard(card2);
        testDeck.addCard(card3);
    ...
    void testFindByTagExcludesUntagged() {
        ...
        Card cardWithTag = new Card(CARD_FRONT_1, CARD_BACK_1);
        Card cardWithoutTag = new Card(CARD_FRONT_2, CARD_BACK_2);
        testDeck.addCard(cardWithTag);
        testDeck.addCard(cardWithoutTag);
    ...
    void testFindByTagDeckIsolation() {
        ...
        // Create second deck
        Deck otherDeck = new Deck("Spanish Vocabulary", "Spanish vocab");
        testUser.addDeck(otherDeck);
        otherDeck = deckRepository.save(otherDeck);

        // Create tags with same name in different decks
        Tag japaneseVerbsTag = new Tag("verbs");
        testDeck.addTag(japaneseVerbsTag);
        japaneseVerbsTag = tagRepository.save(japaneseVerbsTag);

        Tag spanishVerbsTag = new Tag("verbs");
        otherDeck.addTag(spanishVerbsTag);
        spanishVerbsTag = tagRepository.save(spanishVerbsTag);

        // Create cards in both decks
        Card japaneseCard = new Card("食べる", "to eat");
        testDeck.addCard(japaneseCard);
        japaneseCard.addTag(japaneseVerbsTag);
        cardRepository.save(japaneseCard);

        Card spanishCard = new Card("comer", "to eat");
        otherDeck.addCard(spanishCard);
    ...
    /**
     * Tests deleting all cards by clearing the deck's cards collection.
     * With orphanRemoval=true, clearing the collection deletes all cards.
     */
    @Test
    ...
    void testDeleteAll() {
        ...
        // Clear all cards from deck - orphanRemoval will delete them
        testDeck.getCards().clear();
        deckRepository.save(testDeck);
