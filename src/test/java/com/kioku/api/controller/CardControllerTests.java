...
class CardControllerTests {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CardControllerTests.class);
    ...
    void setUp() {
        ...
        // Use Mockito mocks for entities (Spring Boot 4.0 compliant approach)
        testTag = mock(Tag.class);
        when(testTag.getId()).thenReturn(TEST_TAG_ID);
        when(testTag.getName()).thenReturn("Test Tag");

        testCard = mock(Card.class);
        when(testCard.getCardId()).thenReturn(TEST_CARD_ID);
        when(testCard.getFront()).thenReturn(TEST_FRONT);
        when(testCard.getBack()).thenReturn(TEST_BACK);
        when(testCard.getNotes()).thenReturn(TEST_NOTES);
        when(testCard.getTags()).thenReturn(java.util.Collections.emptySet());
    ...
    void testGetDeckCardsSuccess() {
        ...
        when(card2.getFront()).thenReturn("Front 2");
        when(card2.getBack()).thenReturn("Back 2");
        when(card2.getNotes()).thenReturn("Notes 2");
        when(card2.getTags()).thenReturn(java.util.Collections.emptySet());
    ...
    void testUpdateCardSuccess() {
        ...
        Card updatedCard = mock(Card.class);
        when(updatedCard.getCardId()).thenReturn(TEST_CARD_ID);
        when(updatedCard.getFront()).thenReturn(UPDATED_FRONT);
        when(updatedCard.getBack()).thenReturn(UPDATED_BACK);
    ...
    void testAddTagToCardSuccess() {
        ...
        Card cardWithTag = mock(Card.class);
        when(cardWithTag.getCardId()).thenReturn(TEST_CARD_ID);
        when(cardWithTag.getFront()).thenReturn(TEST_FRONT);
        when(cardWithTag.getBack()).thenReturn(TEST_BACK);
        when(cardWithTag.getNotes()).thenReturn(TEST_NOTES);
        when(cardWithTag.getTags()).thenReturn(java.util.Set.of(testTag));
